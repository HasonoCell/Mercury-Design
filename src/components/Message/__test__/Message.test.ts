...
describe('createMessage', () => {
  test('调用方法应该创建对应的 Message 组件', async () => {
    ...
    await rAF()
    expect(document.querySelector('.mr-message')).toBeTruthy()
    instance.destroy()
    await rAF()
    expect(document.querySelector('.mr-message')).toBeFalsy()
  })
  ...
  test('多次调用方法应该创建多个实例', async () => {
    ...
    await rAF()
    const elements = document.querySelectorAll('.mr-message')
